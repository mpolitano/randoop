...
public final class OperationsLog {
  ...
  public static void logStackTrace(Throwable t) {
    ...
    try {
      ...
      PrintWriter pw = new PrintWriter(GenInputsAbstract.coverage_operations_log);
  ...
  public static void logCoveragePrintf(String fmt, Object... args) {
	    if (!isLoggingOn()) {
	      return;
	    }
	    ...
	    try {
	      ...
	      logPrintf("  fmt = %s%n", fmt);
	      logPrintf("  arg types = %s%n", Arrays.toString(argTypes));
	      logCoverageStackTrace(t);
	      return;
	    }

	    try {
	      GenInputsAbstract.coverage_operations_log.write(msg);
	      GenInputsAbstract.coverage_operations_log.flush();
	    } catch (IOException e) {
	      throw new RandoopBug("Exception while writing to log", e);
	    }
	  }
   ...
   * @param t the Throwable whose stack trace to operations_log
   */
  public static void logCoverageStackTrace(Throwable t) {
    if (!isLoggingOn()) {
      return;
    }
